...
public class ChangeMapService {
    ...
    private boolean denyIfEscortingByTauBay(Player p) {
        if (p != null && p.escortRabbit != null) {
            Service.gI().sendThongBao(p, "|7|Bạn đang dắt Thỏ. Không thể dùng Tàu bay!");
            return true;
        }
        return false;
    }
        ...
        try {
            ...
            switch (pl.iDMark.getTypeChangeMap()) {
                case ConstMap.CHANGE_CAPSULE:
                    ...
                    break; // <== thêm break để không rơi qua case khác
...
//            }
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            // NR Namec: cập nhật + drop nếu rời map NRNM
            if (pl.idNRNM != -1) {
                // cập nhật vị trí NRNM
            ...
            // === FIX NRSD: rời map trong giờ NRSD thì reset phù xHPKI/xDame ===
            final boolean leavingMap = pl.zone != null
                    && pl.zone.map != null
                    && pl.zone.map.mapId != zoneJoin.map.mapId;
            if (TimeUtil.isBlackBallWarOpen()) {
                if (pl.effectSkin.xHPKI > 1 && leavingMap) {
                    pl.effectSkin.xHPKI = 1;
                    Service.gI().point(pl);
                }
                if (pl.effectSkin.xDame > 1 && leavingMap) {
                    pl.effectSkin.xDame = 1;
                    Service.gI().point(pl);
                }
            }
            // === END FIX ===
    ...
    public void exitMap(Player player) {
        if (player.zone != null) {
            ...
            // (ĐÃ LOẠI BỎ đoạn code sai dùng mapId ở đây)
    ...
    public Map getMapById(int mapId) {
        for (Map map : Load_Database.MAPS) {
            if (map.mapId == mapId) {
                return map;
            }
        }
