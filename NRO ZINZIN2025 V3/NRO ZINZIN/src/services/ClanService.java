...
public class ClanService {
    ...
    public Clan getClanById(int id) throws Exception {
        return getClanById(0, Load_Database.getNumClan(), id);
    }
    ...
    private Clan getClanById(int l, int r, int id) throws Exception {
        if (l <= r) {
            ...
            Clan clan = null;
            try {
                clan = Load_Database.CLANS.get(m);
            } catch (Exception e) {
    ...
    public List<Clan> getClans(String name) {
        ...
        if (Load_Database.CLANS.size() <= 20) {
            for (Clan clan : Load_Database.CLANS) {
                if (clan.name.contains(name)) {
                    listClan.add(clan);
                }
            }
        } else {
            int n = Util.nextInt(0, Load_Database.CLANS.size() - 20);
            for (int i = n; i < Load_Database.CLANS.size(); i++) {
                Clan clan = Load_Database.CLANS.get(i);
                if (clan.name.contains(name)) {
                    listClan.add(clan);
                }
                if (listClan.size() >= 20) {
    ...
    private void acceptAskJoinClan(Player player, int clanMessageId) {
        ...
        if (clan != null && clan.isLeader(player)) {
            ...
            if (cmg != null && !existInClan) {
                ...
                if (pl == null) {
                    pl = ZINZINSqlFetcher.loadById(plxinvao);
                }
                if (pl != null) {
                    if (pl.iDMark.isHoldBlackBall()) {
