...
public class TaskService {
    ...
    public TaskMain getTaskMainById(Player player, int id) {
        for (TaskMain tm : Load_Database.TASKS) {
            if (tm.id == id) {
    ...
    public SideTaskTemplate getSideTaskTemplateById(int id) {
        if (id != -1) {
            return Load_Database.SIDE_TASKS_TEMPLATE.get(id);
        }
        return null;
    }
    ...
    public KolTaskTemplate getKolTaskTemplateById(int id) {
        if (id != -1) {
            return Load_Database.KOL_TASKS_TEMPLATE.get(id);
        }
        return null;
    }
    ...
    public void changeSideTask(Player player, byte level) {
        ...
        if (player.playerTask.sideTask.leftTask > 0) {
            ...
            SideTaskTemplate temp = Load_Database.SIDE_TASKS_TEMPLATE
                    .get(Util.nextInt(0, Load_Database.SIDE_TASKS_TEMPLATE.size() - 1));
    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                int goldReward = 0;
                Item muoiTieu = ItemService.gI().createNewItem((short) 1545);
                Item dauXanh = ItemService.gI().createNewItem((short) 1548);
                switch (player.playerTask.sideTask.level) {
                    case ConstTask.EASY:
                        goldReward = ConstTask.GOLD_EASY;
                        player.inventory.ruby += 3;
                        break;
                    ...
                    case ConstTask.HELL:
                        ...
                        player.inventory.ruby += 15;
                        muoiTieu.quantity += 4;
                        dauXanh.quantity += 4;
                        break;
                }
                if (InventoryService.gI().getCountEmptyBag(player) > 1) {
                    dauXanh.itemOptions.add(new Item.ItemOption(86, 0));
                    dauXanh.itemOptions.add(new Item.ItemOption(93, 30));
                    dauXanh.itemOptions.add(new Item.ItemOption(174, 2025));
                    dauXanh.itemOptions.add(new Item.ItemOption(30, 0));
                    muoiTieu.itemOptions.add(new Item.ItemOption(86, 0));
                    muoiTieu.itemOptions.add(new Item.ItemOption(93, 30));
                    muoiTieu.itemOptions.add(new Item.ItemOption(174, 2025));
                    muoiTieu.itemOptions.add(new Item.ItemOption(30, 0));
                    InventoryService.gI().sendItemBag(player);
                    InventoryService.gI().addItemBag(player, dauXanh);
                    InventoryService.gI().addItemBag(player, muoiTieu);
    ...
    public ClanTaskTemplate getClanTaskTemplateById(int id) {
        if (id != -1) {
            return Load_Database.CLAN_TASKS_TEMPLATE.get(id);
        }
        return null;
    }
    ...
    public void changeClanTask(Npc npc, Player player, byte level) {
        ...
        if (player.playerTask.clanTask.leftTask > 0) {
            ...
            ClanTaskTemplate temp = Load_Database.CLAN_TASKS_TEMPLATE
                    .get(Util.nextInt(0, Load_Database.CLAN_TASKS_TEMPLATE.size() - 1));
