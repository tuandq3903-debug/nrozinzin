...
public class ChuyenHoaBangVang {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combine.itemsCombine.size() == 2) {
            Item trangBiGoc = player.combine.itemsCombine.get(0);// trangg bị gốc ô 1
            Item trangBiCanChuyenHoa = player.combine.itemsCombine.get(1);// trang bị chuyển hóa ô 2
    ...
    public static void ChuyenHoaBangvang(Player player) {
        if (player.combine.itemsCombine.size() == 2) {
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                ...
                Item trangBiCanChuyenHoa_2 = ItemService.gI().createNewItem(player.combine.itemsCombine.get(1).template.id);// item mới được tạo ra
