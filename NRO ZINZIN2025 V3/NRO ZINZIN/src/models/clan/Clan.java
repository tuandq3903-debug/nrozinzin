...
public class Clan {
    ...
    public void addSMTNClan(Player plOri, long param) {
        for (int i = this.membersInGame.size() - 1; i >= 0; i--) {
            ...
            if (!plOri.equals(pl) && pl != null && Objects.equals(plOri.zone, pl.zone)) {
    ...
    /**
     * Xóa thành viên online khỏi danh sách.
     * Nếu player != null, remove trực tiếp.
     * Nếu cm != null, remove các entry null, sau đó remove mọi player có id == cm.id.
     */
    public void removeMemberOnline(ClanMember cm, Player player) {
        // Bỏ qua nếu list rỗng
        if (membersInGame == null || membersInGame.isEmpty()) {
            return;
        }
        // Nếu có đối tượng Player, remove thẳng
        if (player != null) {
            membersInGame.remove(player);
        }
        if (cm != null) {
            // Loại bỏ các phần tử null tránh NPE
            membersInGame.removeIf(Objects::isNull);
            // Loại bỏ player có id khớp
            membersInGame.removeIf(p -> p.id == cm.id);
        }
    }
    ...
    public Player getPlayerOnline(int playerId) {
        ...
        Player playeroffline = ZINZINSqlFetcher.loadById(playerId);
