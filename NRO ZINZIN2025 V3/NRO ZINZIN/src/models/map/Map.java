...
import models.mob.bigboss_manifest.GaChinCua;
import models.mob.bigboss_manifest.GauTuongCuop;
import models.mob.bigboss_manifest.Hirudegarn;
import models.mob.bigboss_manifest.NguaChinLmao;
import models.mob.bigboss_manifest.Piano;
import models.mob.bigboss_manifest.RobotBaoVe;
import models.mob.bigboss_manifest.VoiChinNga;
import models.mob.bigboss_manifest.VuaBachTuoc;
...
import static server.Load_Database.MAPS;
import static server.Load_Database.MAP_TEMPLATES;
...
public class Map implements Runnable {
    ...
    public void initMob(byte[] mobTemp, byte[] mobLevel, int[] mobHp, short[] mobX, short[] mobY) {
        for (int i = 0; i < mobTemp.length; i++) {
            ...
            Template.MobTemplate temp = Load_Database.getMobTemplateByTemp(mobTempId);
            if (temp != null) {
                Mob mob = new Mob();
                mob.id = i;
    ...
    public static void initMap() {
        int[][] tileTyleTop = RealMap.readTileIndexTileType(ConstMap.TILE_TOP);
        for (Template.MapTemplate mapTemp : MAP_TEMPLATES) {
            int[][] tileMap = RealMap.readTileMap(mapTemp.id);
            int[] tileTop = tileTyleTop[mapTemp.tileId - 1];
            models.map.Map map = new models.map.Map(
