    ...
    private boolean SubThoiVang(Player pl, int quatity) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 457 && item.quantity >= quatity) {
                 services.InventoryService.gI().subQuantityItemsBag(pl, item, quatity);
                return true;
            }
        }
        return false;
    ...
    public void buyItem(Player pl, int id) {
        ...
        if (isBuy) {
            ...
            if (it.isBuy) {
                ...
                ConsignShopManager.gI().save(); // Ensure data is saved immediately after purchase
                openShopKyGui(pl);
            }
        }
    }
        ...
        switch (action) {
            case 1: // hủy vật phẩm
                ...
                if (ConsignShopManager.gI().listItem.remove(it)) {
                    ...
                    InventoryService.gI().sendItemBag(pl);
                    Service.gI().sendMoney(pl);
                    ConsignShopManager.gI().save(); // Save after removing the item
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            ...
            switch (moneyType) {
                case 0:// vàng
                    if (money > 100000 || money < 0) {
                        ...
                        ConsignShopManager.gI().listItem.add(new ConsignItem(newId, it.template.id, (int) pl.id, getTabKiGui(it), money, -1, quantity, (byte) 0, it.itemOptions, false));
                        InventoryService.gI().sendItemBag(pl);
                        success = true;
                    }
                    break;
                case 1:// Ngọc Xanh
                    if (money > 1000000 || money < 0) {
                        ...
                        InventoryService.gI().subQuantityItemsBag(pl, pl.inventory.itemsBag.get(id), quantity);
                        ConsignShopManager.gI().listItem.add(new ConsignItem(newId, it.template.id, (int) pl.id, getTabKiGui(it), -1, money, quantity, (byte) 0, it.itemOptions, false));
                        InventoryService.gI().sendItemBag(pl);
                        success = true;
                    }
                    break;
            ...
            if (success) {
                ...
                ConsignShopManager.gI().save(); // Save immediately after a successful transaction
            }
        } catch (Exception e) {
//             e.printStackTrace();
            Service.gI().sendThongBao(pl, "Có lỗi xảy ra khi ký gửi: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (byte i = 0; i < 5; i++) {
                if (i == 4) {
                    ...
                    msg.writer().writeByte(0);
                    List<ConsignItem> playerItems = getItemCanKiGui(pl);
                    msg.writer().writeByte(playerItems.size());
                    for (int j = 0; j < playerItems.size(); j++) {
                        ConsignItem itk = playerItems.get(j);
                        if (itk == null) {
                            continue;
                        }
                        Item it = ItemService.gI().createNewItem(itk.itemId);
...
//             System.out.println("Error opening shop: " + e.getMessage());
//             e.printStackTrace();
        }
    }
