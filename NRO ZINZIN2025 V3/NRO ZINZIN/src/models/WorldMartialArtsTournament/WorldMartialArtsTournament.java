...
public final class WorldMartialArtsTournament implements Runnable {
    ...
    private void init() {
        ...
        ThreadPoolManager.EXECUTOR.submit(this);
    }
...
//                 e.printStackTrace();
    ...
    public void finish() {
        try {
            ...
            if (plWin != null && plWin.zone != null) {
                ...
                plWin.textThongBaoChangeMap = "Bạn đã thắng tiếp theo.xin chờ tại đây ít phút để thi đấu vòng tiếp theo.";
            ...
            if (plLose != null && plLose.zone != null) {
                ...
                plLose.textThongBaoThua = "Xin lỗi vì điều không mong muốn đã xảy ra.";
            ...
            if (plWin != null) {
                ...
                Service.gI().dropAndPickItem(plWin, 457, 1);
            }
            Service.gI().setPos(npc, npc.location.x, 312);
            npcChat();
    ...
    private void thongBao() {
        switch (typeEnd) {
            ...
            case 2 -> {
                Service.gI().sendThongBao(plWin, "Đối thủ đã kiệt sức, bạn giành chiến thắng.");
            }
            case 3 -> {
                Service.gI().sendThongBao(plWin, "Đối thủ đã bỏ cuộc, %1 giành chiến thắng.");
                Service.gI().sendThongBao(plLose, "Bạn bị xử thua vì đã rời khỏi trận đấu.");
            }
        }
    }
    ...
    private void npcChat() {
        switch (typeEnd) {
            case 0 -> {
                Service.gI().chat(npc, "Thời gian thi đấu đã hết, %1 giành chiến thắng.".replaceAll("%1", plWin.name));
            }
            case 1 -> {
                Service.gI().chat(npc, "Đối thủ đã rời khỏi võ đài, %1 đã giành chiến thắng.".replaceAll("%1", plWin.name));
            }
            case 2 -> {
                Service.gI().chat(npc, "Đối thủ kiệt sức, %1 đã giành chiến thắng.".replaceAll("%1", plWin.name));
            }
            case 3 -> {
                Service.gI().chat(npc, "Đối thủ đã bỏ cuộc, %1 giành chiến thắng.".replaceAll("%1", plWin.name));
            }
        }
    }
