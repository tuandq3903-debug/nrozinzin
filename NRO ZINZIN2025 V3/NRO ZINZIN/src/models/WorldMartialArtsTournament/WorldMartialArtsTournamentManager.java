        ...
        while (!Maintenance.isRunning) {
            try {
                ...
                if (Util.canDoWithTime(lastUpdateTime, 1000)) {
                    ...
                    canReg = TimeUtil.getCurrMin() < ConstDaiHoiVoThuat.MINS_MAX_CAN_REG && tour != -1;
                    if (tour != -1) {
                        gem = ConstDaiHoiVoThuat.tournamentGems[tour];
                        gold = ConstDaiHoiVoThuat.tournamentGolds[tour];
                        cupName = ConstDaiHoiVoThuat.tournamentNames[tour];
                        update();
                    }
...
//                 e.printStackTrace();
    ...
    public void updateChatText() {
        ...
        } else if (TimeUtil.getCurrMin() < ConstDaiHoiVoThuat.MINS_START) {
    ...
    public void update() {
        if (TimeUtil.getCurrMin() > ConstDaiHoiVoThuat.MINS_END) {
            round = 0;
            ...
            if (!listReg.isEmpty() && listTournaments.isEmpty()) {
                ...
                for (int i = listReg.size() - 1; i >= 0; i--) {
                    ...
                    if (pl != null && pl.zone != null) {
                        if (pl.zone.map.mapId != 52) {
                            Service.gI().sendThongBao(pl, "Bạn đã không đến đúng giờ, rất tiếc bạn bị truất quyền tham gia.");
                            listReg.remove(i);
                        }
                    } else {
                        listReg.remove(i);
                ...
                if (listReg.size() % 2 != 0) {
                    ...
                    if (plHup != null) {
                        ...
                        Service.gI().sendThongBao(plHup, "Bạn đã thắng vì đối thủ bỏ cuộc, hãy chờ tại đây để thi đấu vòng tiếp theo.");
                    }
                }
            ...
            if (lastMins != TimeUtil.getCurrMin()) {
                ...
                for (int i = listReg.size() - 1; i >= 0; i--) {
                    try {
                        Player pl = getPlayerById(listReg.get(i));
                        if (pl != null && pl.zone != null) {
                            Service.gI().sendThongBao(pl, "Trận đấu của bạn sẽ diễn ra trong vòng " + (ConstDaiHoiVoThuat.MINS_START - TimeUtil.getCurrMin()) + " phút nữa");
                        }
                    } catch (Exception e) {
                    }
                }
    ...
    public void reward(Player pl) {
        ...
        listReg.clear();
        listWait.clear();
        Service.gI().dropAndPickItem(pl, 457, 1);
        Service.gI().sendThongBao(pl, "Chúc mừng bạn đã vô địch giải đấu! Phần thưởng của bạn là 5 viên đá nâng cấp.");
        for (int i = 220; i < 225; i++) {
            Service.gI().dropAndPickItemDNC(pl, i);
        }
        String msg = "Chúc mừng %1 đã vô địch giải %2!".replaceAll("%1", pl.name).replaceAll("%2", ConstDaiHoiVoThuat.tournamentNames[WorldMartialArtsTournamentService.getTournament()]);
