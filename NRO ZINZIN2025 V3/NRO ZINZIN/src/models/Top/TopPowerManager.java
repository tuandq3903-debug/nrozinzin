...
public class TopPowerManager {
    ...
    private static final TopHungvuong INSTANCE = new TopHungvuong();

    public static TopHungvuong getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DatabaseManager.getConnection(); java.sql.PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(JSON_EXTRACT(data_point, '$[1]') AS UNSIGNED) AS power "
                + "FROM player "
                + "ORDER BY power DESC "
...
//             e.printStackTrace();
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.nPoint.power = rs.getInt("power");
