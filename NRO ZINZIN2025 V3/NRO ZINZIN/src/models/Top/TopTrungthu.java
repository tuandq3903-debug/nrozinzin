 ...
 * @author Admin
 */
public class TopTrungthu {

    @Getter
    private List<Player> list = new ArrayList<>();
    private static final TopTrungthu INSTANCE = new TopTrungthu();

    public static TopTrungthu getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DatabaseManager.getConnection(); java.sql.PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(JSON_EXTRACT(data_tet, '$[0]') AS UNSIGNED) AS trungthu\n "
                + "FROM player\n "
                + "ORDER BY trungthu DESC\n "
...
//             e.printStackTrace();
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.point_trungthu = rs.getInt("trungthu");
