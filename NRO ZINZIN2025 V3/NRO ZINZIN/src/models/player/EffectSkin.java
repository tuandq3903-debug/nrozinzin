...
public class EffectSkin {
    ...
    private static final String[] textZINZIN = new String[]{
        "Ngọc Rồng ZINZIN", "Idol Vip Quá!", "Idol",
        "Trùm Server Đây Rồi!"
    };
...
//             e.printStackTrace();
    ...
    private void updateZINZIN() {
        try {
            if (this.player.nPoint != null && this.player.nPoint.islinhthuydanhbac) {
                if (Util.canDoWithTime(lastTimeZINZIN, 10000)) {
                    List<Player> playersMap = this.player.zone.getNotBosses();
                    for (int i = playersMap.size() - 1; i >= 0; i--) {
                        Player pl = playersMap.get(i);
                        if (pl != null && pl.nPoint != null && !this.player.equals(pl) && !pl.isBoss && !pl.isDie()
                                && Util.getDistance(this.player, pl) <= 200) {
                            Service.gI().chat(pl, textZINZIN[Util.nextInt(0, textZINZIN.length - 1)]);
                        }

                    }
                    this.lastTimeZINZIN = System.currentTimeMillis();
                }
            }
        } catch (Exception e) {
//             e.printStackTrace();
