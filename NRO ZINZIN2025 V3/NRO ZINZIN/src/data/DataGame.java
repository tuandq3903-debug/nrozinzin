...
public class DataGame {
    ...
    public static void updateMap(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(vsMap);
            msg.writer().writeByte(Load_Database.MAP_TEMPLATES.length);
            for (MapTemplate temp : Load_Database.MAP_TEMPLATES) {
                msg.writer().writeUTF(temp.name);
            }
            msg.writer().writeByte(Load_Database.NPC_TEMPLATES.size());
            for (NpcTemplate temp : Load_Database.NPC_TEMPLATES) {
                msg.writer().writeUTF(temp.name);
            ...
            msg.writer().writeByte(Load_Database.MOB_TEMPLATES.size());
            for (MobTemplate temp : Load_Database.MOB_TEMPLATES) {
    ...
    public static void updateSkill(MySession session) {
        ...
        try {
            ...
            msg.writer().writeByte(Load_Database.NCLASS.size());
            for (NClass nClass : Load_Database.NCLASS) {
...
//             e.printStackTrace();
    ...
    public static void sendBgItemVersion(MySession session) {
        ...
        try {
            msg = new Message(-93);
            msg.writer().writeShort(Load_Database.BG_ITEMS.size());
            for (BgItem bgItem : Load_Database.BG_ITEMS) {
    ...
    public static void sendHeadAvatar(Message msg) {
        try {
            msg.writer().writeShort(Load_Database.HEAD_AVATARS.size());
            for (HeadAvatar ha : Load_Database.HEAD_AVATARS) {
    ...
    public static void sendImageByName(MySession session, String imgName) {
        ...
        try {
            ...
            msg.writer().writeByte(Load_Database.getNFrameImageByName(imgName));
    ...
    public static void sendRes(MySession session) {
     ...
     public static void sendLinkIP(MySession session) {
        ...
        try {
            msg = new Message(-29);
            msg.writer().writeByte(2);
            msg.writer().writeUTF(Config.LINK_IP_PORT + ",0,0");
            msg.writer().writeByte(1);
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
