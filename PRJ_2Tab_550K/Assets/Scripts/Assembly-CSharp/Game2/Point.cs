namespace Game2
{
	public class Point
	{
		...
		public void create_Speed(int dx, int dy)
		{
			...
			int num = Res.getDistance(dx, dy) / vMax;
			if (num == 0)
			{
				num = 1;
			}
		...
		public void moveTo_xy(int toX, int toY)
		{
			...
			else if (Res.abs(y - toY) < Res.abs(vy))
			{
				y = toY;
				vy = 0;
			}
		...
		public void paint_Arrow(mGraphics g, FrameImage frm, int anchor, bool isCountFr)
		{
			if (frm != null)
			{
				...
				int num2 = 3;
				int num3 = ((frm.nFrame <= 3) ? (f % num) : ((f / num2 % 2 != 0) ? 3 : 0));
				int idx = num * mImageArrow[frame] + num3;
				if (frm.nFrame < 3)
				{
					idx = f / num2 % frm.nFrame;
				}
				if (isCountFr)
				{
					idx = f / num2 % frm.nFrame;
