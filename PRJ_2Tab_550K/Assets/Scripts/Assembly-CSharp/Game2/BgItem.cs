namespace Game2
{
	public class BgItem
	{
		...
		public void paint(mGraphics g)
		{
			...
			int cmx = GameScr.cmx;
			int cmy = GameScr.cmy;
			Image image = ((layer == 2 || layer == 4) ? ((Image)imgNew.get(idImage + string.Empty)) : (isNotBlend() ? ((Image)imgNew.get(idImage + string.Empty)) : ((Image)imgNew.get(idImage + "blend" + layer))));
