		...
		public void update()
		{
			try
			{
				...
				case 12:
				{
					for (int n = 0; n < sum; n++)
					{
						if (n % 3 != 0 && typeEff != 12 && TileMap.tileTypeAt(x[n], y[n] - GameCanvas.transY, 2))
						{
							activeEff[n] = true;
						}
						if (n % 3 == 0 && y[n] > GameCanvas.h + GameScr.cmy)
						{
							x[n] = Res.random(-10, GameCanvas.w + 300) + GameScr.cmx;
							y[n] = Res.random(-100, 0) + GameScr.cmy;
						}
						if (!activeEff[n])
						{
							y[n] += vy[n];
				...
				case 9:
				{
					for (int l = 0; l < this.num; l++)
					{
						...
						if (x[l] < -vx[l])
						{
							wP[l] = Res.abs(Res.random(1, 3));
							vx[l] = wP[l];
							x[l] = GameCanvas.w + vx[l];
						}
					}
					break;
				}
				case 10:
				{
					...
					{
						x[i] -= vx[i];
						if (x[i] < -vx[i] + GameScr.cmx)
						{
							x[i] = GameCanvas.w + vx[i] + GameScr.cmx;
						}
					}
					break;
				}
