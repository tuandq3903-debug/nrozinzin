...
namespace Game2
{
    public class Panel : IActionListener, IChatable
    {
        ...
        // "Tried to load" flags to avoid loadImage() being called every frame for missing files
        private bool imgo_1_loaded = false;
        private bool imgo_2_loaded = false;
        private bool imgo_3_loaded = false;
        ...
        // Cache for paintInventory stats (recomputed every 30 frames to avoid per-frame lag)
        private int _invCacheTick = -1;
        private int _invCacheHash = -1;
        private int _cacheTotalHP = 0;
        private int _cacheTotalMP = 0;
        private int _cacheTotalDame = 0;
        private int _cacheTotalDameCM = 0;
        private int _cacheTotalHutHP = 0;
        private int _cacheTotalHutMP = 0;
        private bool _cacheChonglanh = false;
        private bool _cacheKhangtdhs = false;
...
// Compute item stats every 30 frames OR when item count changes (cache hit avoids per-frame lag)
int curHash = (arrItemBody.Length << 16) | arrItemBag.Length;
if (_invCacheTick < 0 || (GameCanvas.gameTick - _invCacheTick) > 30 || _invCacheHash != curHash)
{
    _cacheTotalHP = 0;
    _cacheTotalMP = 0;
    _cacheTotalDame = 0;
    ...
    _cacheTotalHutHP = 0;
    _cacheTotalHutMP = 0;
    _cacheChonglanh = false;
    _cacheKhangtdhs = false;
    for (int i = 0; i < arrItemBody.Length; i++)
    {
        ...
        for (int j = 0; j < item.itemOption.Length; j++)
        {
            ...
            if (opt != null)
            {
                if (opt.optionTemplate.id == 77) _cacheTotalHP += opt.param;
                else if (opt.optionTemplate.id == 103) _cacheTotalMP += opt.param;
                else if (opt.optionTemplate.id == 50) _cacheTotalDame += opt.param;
                else if (opt.optionTemplate.id == 5) _cacheTotalDameCM += opt.param;
                else if (opt.optionTemplate.id == 95) _cacheTotalHutHP += opt.param;
                else if (opt.optionTemplate.id == 96) _cacheTotalHutMP += opt.param;
                else if (opt.optionTemplate.id == 106) _cacheChonglanh = true;
                else if (opt.optionTemplate.id == 116) _cacheKhangtdhs = true;
            }
        }
    }
    _invCacheTick = GameCanvas.gameTick;
    _invCacheHash = curHash;
}
int totalHP = _cacheTotalHP;
int totalMP = _cacheTotalMP;
int totalDame = _cacheTotalDame;
int totalDameCM = _cacheTotalDameCM;
int totalHutHP = _cacheTotalHutHP;
int totalHutMP = _cacheTotalHutMP;
bool chonglanh = _cacheChonglanh;
bool khangtdhs = _cacheKhangtdhs;
        ...
        public void paintTask(mGraphics g)
        {
            ...
            else if (indexMenu == 1)
            {
                ...
                for (int m = 0; m < Char.myCharz().taskOrders.size(); m++)
                {
                    ...
                    {
                        mFont.tahoma_7_grey.drawString(g, ((taskOrder.taskId != 0) ? mResources.KILLBOSS : mResources.KILL) + " " + Mob.arrMobTemplate[taskOrder.killId].name + " (" + taskOrder.count + "/" + taskOrder.maxCount + ")", xstart + 5, yPaint += 12, 0);
                    }
                    indexRowMax += 3;
                    inforW = popupW - 25;
