namespace Game2
{
	public class PlayerDart
	{
		...
		public PlayerDart(Char charBelong, int dartType, SkillPaint sp, int x, int y)
		{
			...
			{
				IMapObject charFocus = charBelong.mobFocus;
				mapObject = charFocus;
			}
			IMapObject mapObject2 = mapObject;
			setAngle(Res.angle(mapObject2.getX() - x, mapObject2.getY() - y));
		}
		...
		public void update()
		{
			...
			for (int i = 0; i < info.nUpdate; i++)
			{
				...
				int num = ((charBelong.getX() <= mapObject2.getX()) ? (-10) : 10);
				dx = mapObject2.getX() + num - x;
				dy = mapObject2.getY() - mapObject2.getH() / 2 - y;
				life++;
