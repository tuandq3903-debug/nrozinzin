	...
	private void getKey(Message message)
			...
			private Message readMessage()
			{
				try
				{
					...
					if (b == -32 || b == -66 || b == -28 || b == 11 || b == -67 || b == -74 || b == -87 || b == 66 || b == 12 )
					{
				...
				catch (IOException ex) when (checkConnected())
				{
					Debug.Log(ex.StackTrace);
					connected = false;
				}
				catch (ObjectDisposedException ex) when (checkConnected())
				{
					Debug.Log(ex.StackTrace);
					connected = false;
				}
				catch (SocketException ex) when (checkConnected())
				{
					Debug.Log(ex.StackTrace);
					connected = false;
				}
