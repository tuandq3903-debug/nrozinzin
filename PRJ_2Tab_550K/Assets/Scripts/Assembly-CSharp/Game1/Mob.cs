	...
	public class Mob : IMapObject
	{
		...
		public virtual void update()
		{
			...
			case 5:
				...
				else if (arrMobTemplate[templateId].type == 4)
				{
					ty++;
					wt++;
					fy += ((!wy) ? 1 : (-1));
		...
		{
			if ((templateId < 58 || templateId > 65) && templateId != 67)
			{
				return templateId == 68;
			}
			return true;
		}
		...
		private void updateInjure()
		{
			...
			{
				if ((injureBy != null && injureThenDie) || hp == 0L)
				{
					status = 1;
					p2 = injureBy.cdir << 1;
					p1 = -3;
		...
		private void updateMobStandWait()
		{
			...
			sbyte type = arrMobTemplate[templateId].type;
			if (type > 3)
			{
				if (type - 4 <= 1)
				{
		...
		public void updateMobAttack()
		{
			...
			if (p1 == 0)
			{
				int num = ((cFocus == null) ? mobToAttack.x : cFocus.cx);
				int num2 = ((cFocus == null) ? mobToAttack.y : cFocus.cy);
				if (!isNewMod())
				{
		...
		public bool isPaint()
		{
			if (x >= GameScr.cmx && x <= GameScr.cmx + GameScr.gW && y >= GameScr.cmy && y <= GameScr.cmy + GameScr.gH + 30 && arrMobTemplate[templateId] != null && arrMobTemplate[templateId].data != null && arrMobTemplate[templateId].data.img != null)
			{
				return status != 0;
			}
			return false;
		...
		{
			if (arrMobTemplate[templateId] != null && arrMobTemplate[templateId].data != null)
			{
				return status != 0;
			}
			return false;
		...
		public virtual void paint(mGraphics g)
		{
			...
			{
				int num3 = per_tem;
				int num4 = ((GameCanvas.gameTick % 6 <= 3) ? offset : offset++);
				num2 = imageWidth * (per_tem = num3 - num4) / 100;
				if (per_tem <= 0)
				{
					per_tem = 0;
				}
		...
		private bool isTypeNewMod()
		{
			if (arrMobTemplate[templateId].data != null)
			{
				return arrMobTemplate[templateId].data.typeData == 2;
			}
			return false;
		}
	}
